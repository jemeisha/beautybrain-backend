...
def skin_detection(original):
    #original = read_image("C:\\Users\\ROG\\Desktop\\Skyn-main\\Skyn-main\\ML\\Skin_metrics\\Skin_tone\\public\\test images\\AcneEntireFace-refill-cropped.jpg")
...
def read_image(dir):
    ...
    image_path = dir
    img_np=np.frombuffer(dir,np.uint8)
    img_BGR=cv2.imdecode(img_np,3)
...
def skin_cluster(dframe):
    ...
    print("kmean l: ",np.unique(kmeans.labels_))
    print("kmean l: ",(kmeans.labels_).size)
...
def cluster_matrix(dframe, dframe_removed, skin_cluster_label, height, width):
    ...
    # print("dframe ",(dframe['cluster'] ==
    #                      skin_cluster_label).astype(int))
